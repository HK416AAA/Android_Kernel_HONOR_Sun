...
struct blkcg_gq {
	...
	/* unsigned int weight; weight = lower_32_bits(android_oem_data1) */
	ANDROID_OEM_DATA(1);
};
...
struct blkcg {
	...
	/* unsigned int weight;	weight = lower_32_bits(android_oem_data1) */
	/* unsigned int type;	type   = upper_32_bits(android_oem_data1) */
	ANDROID_OEM_DATA(1);
};
...
#if defined(CONFIG_HONOR_BLK_CGROUP_IOSMART) || defined(CONFIG_HONOR_BLK_CGROUP_IOLIMIT)
...
static inline bool blk_is_topio(void)
{
	...
	rcu_read_lock();
	p_oem_data = (struct blkcg_oem_data *)
				&task_blkcg(current)->android_oem_data1;
	ret = (p_oem_data->type == BLK_THROTL_TA);
	rcu_read_unlock();
	return ret;
}
#endif
