...
static void tcp_rcv_rtt_update(struct tcp_sock *tp, u32 sample, int win_dep)
{
	...
	trace_android_vh_ogki_tcp_rcv_rtt_update(tp, sample, win_dep);
...
static void tcp_rtt_estimator(struct sock *sk, long mrtt_us)
{
	...
	trace_android_vh_ogki_tcp_srtt_estimator(sk);
}
...
no_ack:
			trace_android_vh_ogki_tcp_rcv_estab_fastpath(sk);
			if (eaten)
				kfree_skb_partial(skb, fragstolen);
			tcp_data_ready(sk);
			return;
...
step5:
	...
	trace_android_vh_ogki_tcp_rcv_estab_slowpath(sk);
	return;
