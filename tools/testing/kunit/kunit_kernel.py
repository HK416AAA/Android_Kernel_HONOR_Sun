...
class LinuxSourceTree:
	...
	def is_config_not_set(self, s) -> bool:
		# 定义正则表达式模式
		pattern = r'^# CONFIG_.* is not set$'
	...
	def validate_config(self, build_dir: str) -> bool:
		...
		all_missing_is_not_set=True
		for e in missing:
			if not self.is_config_not_set(str(e)):
				all_missing_is_not_set=False
				break

		if all_missing_is_not_set:
			return True
