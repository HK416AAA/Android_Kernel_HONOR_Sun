...
#endif
#if (defined CONFIG_HONOR_DIE_CATCH) && IS_ENABLED(CONFIG_HN_ZEROHUNG)
...
#ifdef CONFIG_HONOR_DIE_CATCH
		catch_flags = p->signal->android_oem_data1;
		to_pid = p->pid;
		if ((sig == SIGKILL || sig == SIGTERM) && (catch_flags & EXIT_CATCH_FORAPP_FLAG)) {
			p->signal->android_oem_data1 = 0;
...
#if (defined CONFIG_HONOR_DIE_CATCH) && IS_ENABLED(CONFIG_HN_ZEROHUNG)
	cur_flags = current->signal->android_oem_data1;
	if ((catch_flags & EXIT_CATCH_FORAPP_FLAG) &&
		!(cur_flags & KILL_CATCH_OLD_FLAG) && hung_wp_screen_getbl()) {
