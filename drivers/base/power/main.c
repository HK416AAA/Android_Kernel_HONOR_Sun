...
#define THREAD_DEBUG_THRESHOLD_REPORT (unsigned long long)1500
				  ...
				  void *cb, int error)
{
	...
	rettime = ktime_get();
	notifier_time = (unsigned long long)ktime_us_delta(rettime, calltime);
	if(notifier_time > THREAD_DEBUG_THRESHOLD_REPORT) {
		dev_info(dev, "calling %pS @ %i, parent: %s\n", cb, task_pid_nr(current),
			dev->parent ? dev_name(dev->parent) : "none");
		dev_info(dev, "report %pS returned %d after %Ld usecs\n", cb, error, notifier_time);
	}
}
