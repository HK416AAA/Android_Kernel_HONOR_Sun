...
struct uid_entry {
	...
	u64 utime;
	u64 stime;
#ifdef CONFIG_HONOR_CPU_FREQ_POWER_STAT
	u64 power;
#endif
	int state;
	struct io_stats io[UID_STATE_SIZE];
...
static void calc_uid_cputime(struct uid_entry *uid_entry,
			u64 *total_utime, u64 *total_stime, u64 *total_power)
#else
static void calc_uid_cputime(struct uid_entry *uid_entry,
	...
	for_each_process(p) {
		...
		for_each_thread(p, t) {
			...
			if (!(t->flags & PF_EXITING)) {
				 ...
				 * time and power.
				 */
				if (t->android_oem_data1[4] == ULLONG_MAX)
					continue;
				*total_power += t->android_oem_data1[4];
#endif
...
#ifdef CONFIG_HONOR_CPU_FREQ_POWER_STAT
			u64 total_power = uid_entry->power;
			calc_uid_cputime(uid_entry, &total_utime, &total_stime, &total_power);
...
#ifdef CONFIG_HONOR_CPU_FREQ_POWER_STAT
			usw->power = task->android_oem_data1[4];
			task->android_oem_data1[4] = ULLONG_MAX;
#endif
...
#ifdef CONFIG_HONOR_CPU_FREQ_POWER_STAT
	uid_entry->power += task->android_oem_data1[4];
	task->android_oem_data1[4] = ULLONG_MAX;
#endif
