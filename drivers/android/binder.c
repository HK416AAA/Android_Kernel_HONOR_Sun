...
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
#include <linux/trace_clock.h>
#endif

#ifdef CONFIG_HONOR_BINDER_TRANSACTION_DESCRIPTOR
...
/**
 * Normal length
 * of [header(16bytes) + descriptor(>20bytes) + data(>6bytes)]
 * is bigger than 40
 */
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	struct binder_transaction *t;
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
	struct hn_binder_transinfo *hn_transinfo;
#endif
	struct binder_work *w;
...
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
	t = kzalloc(sizeof(*t) + sizeof(struct hn_binder_transinfo), GFP_KERNEL);
#else
...
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
	t->android_oem_data1 = (u64)(t + 1);
	hn_transinfo = (struct hn_binder_transinfo *) (t->android_oem_data1);
#endif
	...
	if (!reply && !(tr->flags & TF_ONE_WAY)) {
		t->from = thread;
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
		hn_transinfo->async_from_pid = -1;
		hn_transinfo->async_from_tid = -1;
		hn_transinfo->alloc_buff_size = (u64)tr->data_size;
#endif
	} else {
		t->from = NULL;
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
		hn_transinfo->async_from_pid = thread->proc->pid;
		hn_transinfo->async_from_tid = thread->pid;
		hn_transinfo->alloc_buff_size = (u64)tr->data_size;
#endif
	}
	t->from_pid = proc->pid;
...
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
		hn_transinfo->timestamp_binder = ((struct hn_binder_transinfo *)
			in_reply_to->android_oem_data1)->timestamp_binder;
#endif
...
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
		hn_transinfo->timestamp_binder = binder_clock();
#endif
		binder_inner_proc_unlock(proc);
				 ...
				 struct binder_thread *thread)
{
...
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_BRIEF
	struct hn_binder_transinfo *hn_transinfo = NULL;
#endif
	binder_inner_proc_lock(thread->proc);
	/*
	...
	while (t) {
		...
		if (t->to_thread == thread) {
			...
			hn_transinfo->async_from_pid = -1;
			hn_transinfo->async_from_tid = -1;
			hn_transinfo->alloc_buff_size = 0;
#endif
			t = t->from_parent;
		} else
			BUG();
...
static bool validate_descriptor(struct descriptor_struct *descriptor, int len)
{
	...
	if (descriptor->header[0] != 0x54 || descriptor->header[1] != 0x53 ||
		descriptor->header[2] != 0x59 || descriptor->header[3] != 0x53 ||
		interface_len < 8) {
			return false;
	}
						...
						char *interface)
{
	...
	copy_length = data_size >= sizeof(struct descriptor_struct) ?
					sizeof(struct descriptor_struct) - 2 : data_size;
	memset((char *)&raw_descriptor, 0, sizeof(struct descriptor_struct));

	/* copy interface info from binder_buffer */
	if (honor_binder_alloc_do_copy_from_buffer(alloc, buffer, 0,
					(void *)&raw_descriptor, copy_length)) {
		return;
	}
	if (validate_descriptor(&raw_descriptor, copy_length - INTERFACE_DATA_OFFSET)) {
		u32 interface_len = raw_descriptor.interface_length;
		int max_len = interface_len >= MAX_LEN_INTERFACE ?
				MAX_LEN_INTERFACE - 1 : interface_len;
				...
				struct binder_proc *proc)
{
	...
	from_pid = t->from ? (t->from->proc ? t->from->proc->pid : 0) :
			hn_transinfo->async_from_pid;
	from_tid = t->from ? t->from->pid : hn_transinfo->async_from_tid;
	to_pid = to_proc ? to_proc->pid : 0;
#ifdef CONFIG_HONOR_BINDER_TRANSACTION_DESCRIPTOR
	/* Ensure that to_proc is lokced while reading buffer */
	if (to_proc && (proc == to_proc)) {
		if (t->buffer) {
			get_interface_from_binder_buffer(t->buffer, &to_proc->alloc,
								(char *)interface);
		}
	}
#endif
	seq_printf(m,
		   ...
		   interface,
		   t->code,
		   timestamp_binder > hn_transinfo->timestamp_binder ?
			(timestamp_binder - hn_transinfo->timestamp_binder) / 1000000000 : 0,
		   timestamp_binder > hn_transinfo->timestamp_binder ?
			(timestamp_binder - hn_transinfo->timestamp_binder) % 1000000000 : 0,
		   hn_transinfo->alloc_buff_size);
	spin_unlock(&t->lock);
}
				...
				u64 timestamp_binder)
{
	...
	for (n = rb_first(&proc->threads); n != NULL; n = rb_next(n)) {
		...
		while (t) {
			if (t->from == thread) {
				...
				print_binder_transaction_brief_ilocked(m, "\t",
							t, timestamp_binder, proc);
				t = t->to_parent;
			} else {
				t = NULL;
			}
	...
	for (n = rb_first(&proc->nodes); n != NULL; n = rb_next(n)) {
		...
		binder_node_inner_lock(node);
		list_for_each_entry(w, &node->async_todo, entry)
			print_binder_work_transaction_nilocked(m, "async\t",
							w, timestamp_binder, proc);
		binder_node_inner_unlock(node);
		last_node = node;
		binder_inner_proc_lock(proc);
	}
