...
ufshcd_wait_for_uic_cmd(struct ufs_hba *hba, struct uic_command *uic_cmd)
{
	...
	if (wait_for_completion_timeout(&uic_cmd->done,
		...
		ufs_dsm_report(hba, DSM_UFS_UIC_CMD_ERR, "uic cmd 0x%x with arg3 0x%x completion timeout", uic_cmd->command, uic_cmd->argument3);
#endif
		if (!uic_cmd->cmd_active) {
			dev_err(hba->dev, "%s: UIC cmd has been completed, return the result\n",
				__func__);
#ifdef CONFIG_HONOR_UFS_DSM
			ufs_dsm_report(hba, DSM_UFS_UIC_CMD_ERR, "UIC cmd has been completed, return the result");
#endif
			ret = uic_cmd->argument2 & MASK_UIC_COMMAND_RESULT;
		}
	}
...
#ifdef CONFIG_HONOR_UFS_DSM
	if (ret)
		ufs_dsm_report(hba, DSM_UFS_UIC_CMD_ERR, "UIC Cmd Error");
#endif
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	err = ufshcd_state_of_ffu_check(hba, cmd);
#endif
...
ufshcd_dev_cmd_completion(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	case UPIU_TRANSACTION_NOP_IN:
		if (hba->dev_cmd.type != DEV_CMD_TYPE_NOP) {
			...
			dev_err(hba->dev, "%s: unexpected response %x\n",
					__func__, resp);
#ifdef CONFIG_HONOR_UFS_DSM
			ufs_dsm_report(hba, DSM_UFS_UIC_TRANS_ERR, "unexpected response %x", resp);
#endif
		}
		break;
	...
	case UPIU_TRANSACTION_REJECT_UPIU:
		...
		dev_err(hba->dev, "%s: Reject UPIU not fully implemented\n",
				__func__);
#ifdef CONFIG_HONOR_UFS_DSM
		ufs_dsm_report(hba, DSM_UFS_UIC_TRANS_ERR, "Reject UPIU not fully implemented");
#endif
		break;
	...
	default:
		...
		dev_err(hba->dev, "%s: Invalid device management cmd response: %x\n",
				__func__, resp);
#ifdef CONFIG_HONOR_UFS_DSM
		ufs_dsm_report(hba, DSM_UFS_UIC_TRANS_ERR, "Invalid device management cmd response: %x", resp);
#endif
		break;
	}
...
#ifdef CONFIG_HONOR_UFS_DSM
		ufs_dsm_report(hba, DSM_UFS_UIC_CMD_ERR,
			"pwr ctrl cmd 0x%x with mode 0x%x completion timeout", cmd->command, cmd->argument3);
#endif
...
#ifdef CONFIG_HONOR_UFS_DSM
			ufs_dsm_report(hba, DSM_UFS_UIC_CMD_ERR,
				"Power Mode Change operation has been completed, go check UPMCRS");
#endif
			goto check_upmcrs;
		}
...
#ifdef CONFIG_HONOR_UFS_DSM
		ufs_dsm_report(hba, DSM_UFS_UIC_CMD_ERR,
			"pwr ctrl cmd 0x%x failed, host upmcrs:0x%x", cmd->command, status);
#endif
...
int ufshcd_link_recovery(struct ufs_hba *hba)
{
	...
	if (ret)
		dev_err(hba->dev, "%s: link recovery failed, err %d",
			__func__, ret);
#ifdef CONFIG_HONOR_UFS_DSM
		ufs_dsm_report(hba, DSM_UFS_LINKUP_ERR, "link recovery failed, err %d", ret);
#endif
	return ret;
}
...
#ifdef CONFIG_HONOR_UFS_DSM
	if (ret)
		ufs_dsm_report(hba, DSM_UFS_ENTER_OR_EXIT_H8_ERR, "hibern8 enter failed. ret = %d", ret);
#endif
	return ret;
}
...
#ifdef CONFIG_HONOR_UFS_DSM
			ufs_dsm_report(hba, DSM_UFS_LINKUP_ERR, "Device not present");
#endif
			ret = -ENXIO;
			goto out;
		}
...
#ifdef CONFIG_HONOR_UFS_DSM
		ufs_dsm_report(hba, DSM_UFS_LINKUP_ERR, "link startup failed %d", ret);
#endif
		ufshcd_print_host_state(hba);
		ufshcd_print_pwr_info(hba);
		ufshcd_print_evt_hist(hba);
	...
	if (hba->ufs_device_wlun) {
		...
		if (!link) {
			...
			ufs_dsm_report(hba, DSM_UFS_LINKUP_ERR,
				"Failed establishing link - %s", dev_name(&hba->ufs_device_wlun->sdev_gendev));
#endif
			return;
		}
		hba->luns_avail--;
...
#ifdef CONFIG_HONOR_UFS_DSM
			ufs_dsm_report(hba, DSM_UFS_UTP_ERR, "Reject UPIU not fully implemented");
#endif
			break;
		default:
			dev_err(hba->dev,
...
#ifdef CONFIG_HONOR_UFS_DSM
		ufs_dsm_report(hba, DSM_UFS_UTP_ERR,
			"OCS error from controller = %x for tag %d", ocs, lrbp->task_tag);
#endif
		ufshcd_print_evt_hist(hba);
		ufshcd_print_host_state(hba);
		break;
...
#ifdef CONFIG_HONOR_UFS_DSM
	if (err) {
		ufs_dsm_report(hba, DSM_UFS_DEVICE_RESET_ERR, "UFS Occur Device Reset and Restore");
	}
#endif
	return err;
}
...
static int ufshcd_device_init(struct ufs_hba *hba, bool init_dev_params)
{
	...
	if (ufshcd_ffu_get_device_data(hba)) {
		dev_info(hba->dev, "%s: ufshcd_ffu_get_device_data fail\n", __func__);
	}
#endif
...
#ifdef CONFIG_HONOR_UFS_DSM
void ufshcd_linkup_err_dmd(struct ufs_hba *hba)
{
	...
	if ((hba->max_pwr_info.info.lane_rx < avail_lane_rx) ||
		...
		dev_err(hba->dev,
			"ufs line number is less than avail "
			"rx=%d, tx=%d, avail_rx=%d, avail_tx=%d\n",
			hba->max_pwr_info.info.lane_rx,
			hba->max_pwr_info.info.lane_tx, avail_lane_rx,
			avail_lane_tx);

		ufs_dsm_report(
			hba, DSM_UFS_LINKUP_ERR,
			"ufs line number is less than avail rx=%d, tx=%d, avail_rx=%d, avail_tx=%d",
			hba->max_pwr_info.info.lane_rx,
			hba->max_pwr_info.info.lane_tx, avail_lane_rx,
			avail_lane_tx);
	}
...
void ufshcd_eh_timed_out_dmd(struct scsi_cmnd *scmd, struct ufs_hba *hba)
{
	unsigned long flags;
	struct Scsi_Host *host;
	int index;
	...
	if (!scmd || !scmd->device || !scmd->device->host || !hba)
		return;
	...
	if (true == found) {
		...
		*/
		...
		if (ufs_timeout_count > UFS_TIMEOUT_SERIOUS_THRESHOLD)
			ufs_dsm_report(hba, DSM_UFS_TIMEOUT_SERIOUS,
				       "UFS TimeOut Serious Error");

		ufs_dsm_report(hba, DSM_UFS_TIMEOUT_ERR, "UFS TimeOut Error");
	} else {
		dev_warn(
			hba->dev,
			"scsi cmd[%x] with tag[%x] is timeout which can't be found.",
			scmd->cmnd[0], scsi_cmd_to_rq(scmd)->tag);
	}
}
#endif
...
#ifdef CONFIG_HONOR_UFS_DSM
	ufshcd_eh_timed_out_dmd(scmd, hba);
#endif
...
void ufshcd_remove(struct ufs_hba *hba)
{
	...
	ufshcd_hba_stop(hba);
#ifdef CONFIG_HONOR_SCSI_UFS_FFU
	wakeup_source_trash(&hba->ffu_lock);
#endif
	ufshcd_hba_exit(hba);
}
...
#ifdef CONFIG_HONOR_SCSI_UFS_FFU
	wakeup_source_init(&hba->ffu_lock, "ffu_wakelock");
#endif
...
#ifdef CONFIG_HONOR_SCSI_UFS_FFU
int hn_ufshcd_reset_and_restore(struct ufs_hba *hba)
{
	return ufshcd_reset_and_restore(hba);
}
EXPORT_SYMBOL_GPL(hn_ufshcd_reset_and_restore);

int hn_ufs_get_device_desc(struct ufs_hba *hba)
{
	return ufs_get_device_desc(hba);
}
EXPORT_SYMBOL_GPL(hn_ufs_get_device_desc);

void hn_ufs_fixup_device_setup(struct ufs_hba *hba)
{
	ufs_fixup_device_setup(hba);
}
EXPORT_SYMBOL_GPL(hn_ufs_fixup_device_setup);
#endif
