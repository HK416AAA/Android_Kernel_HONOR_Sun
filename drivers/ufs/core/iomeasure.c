...
#define REQ_HW_VIP           \
       ((__force blk_opf_t)(1ULL <<__REQ_HW_VIP)) /* vip for io scheduler and device */
...
#ifdef CONFIG_HONOR_BLK_CGROUP_IOSMART
...
#define FS_DATAREAD_LATENCY_THRESHOLD (48000) /* us */
#define FS_DATAWRITE_LATENCY_THRESHOLD (64000) /* us */
#define BLOCK_LATENCY_THRESHOLD (32000000) /* ns */
#define FS_DATAREAD_LATENCY "fs_data_read"
#define FS_DATAWRITE_LATENCY "fs_data_write"
#define BLOCK_LATENCY "block_insert_issue"
#define LONG_IO_INTERVAL 10
#define MAX_COMM_LEN 16
static unsigned int iolatency_usertype;
static unsigned long fsread_io_num;
static unsigned long fswrite_io_num;
static unsigned long block_io_num;
...
#define iom_pr_info(fmt, arg...) \
	pr_info("[io_measure] %s," fmt, __func__, ##arg)
...
typedef void (*ioml_add_func)(void *iom_latency, u32 chunksz_idx, u32 time_us,
			      u32 io_type);
...
static struct io_measure_ctrl *iom_ctrl;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
static struct iom_scsi_time_store *iom_scsi_time;
#endif
static DEFINE_MUTEX(iom_enable_lock);

struct task_struct *find_task_by_vpid_safely(pid_t nr)
{
	...
	if (current->flags & PF_EXITING) {
		iom_pr_info(
			"current task is exiting. cannot call find_task_by_vpid\n");
		rcu_read_unlock();
		return NULL;
	}
...
static void io_measure_latency_add(void *iom_latency, u32 chunksz_idx,
				   u32 time_us, u32 io_type)
{
	struct io_measure_latency *iom_ltcy =
		(struct io_measure_latency *)iom_latency;
	u32 time_idx;
	...
	/* max io latency */
	if (unlikely((u64)time_us >
		     iom_ltcy->latency_stat[chunksz_idx][MAX_LATENCY][io_type]))
		iom_ltcy->latency_stat[chunksz_idx][MAX_LATENCY][io_type] =
			(u64)time_us;

	/* io average latency stat */
	iom_ltcy->latency_avg[chunksz_idx][io_type].count++;
	iom_ltcy->latency_avg[chunksz_idx][io_type].latency_total +=
		(u64)time_us;
...
static void io_measure_latency_get(void *iom_latency, char *buf, u32 buf_len)
{
	struct io_measure_latency *iom_ltcy =
		(struct io_measure_latency *)iom_latency;
	int i, j, k;
	...
	/* Latency title */
	offset = scnprintf(buf, buf_len, "Latency[%s]:\t",
			   (char *)iom_ltcy->log_tag);
	for (i = 0; i < TIME_DIV_NUM - 2; i++) {
		offset += scnprintf(buf + offset, buf_len - offset, "%d\t",
				    IOM_TIME_IDX_TO_MS(i));
	}
	offset += scnprintf(buf + offset, buf_len - offset,
			    "Oversize\tMax(us)\tAvg(us)");

	/* Latency stat */
	for (i = 0; i < IO_CHUNK_NUM; i++) {
		offset += scnprintf(buf + offset, buf_len - offset,
				    "\nIO[%u]:\t",
				    IOM_BYTE_TO_KB(iom_chunksize_array[i]));
		...
		for (k = 0; k < IO_TP_NUM; k++)
			offset += scnprintf(
				buf + offset, buf_len - offset,
				(k + 1 == IO_TP_NUM) ? "%llu" : "%llu/",
				(iom_ltcy->latency_avg[i][k].count == 0) ?
					0 :
					iom_ltcy->latency_avg[i][k]
							.latency_total /
						iom_ltcy->latency_avg[i][k]
							.count);
	}
 ...
 */
static void io_measure_latency_init(struct io_measure_latency *iom_latency,
				    char *tag)
{
		...
		for (j = 0; j < TIME_DIV_NUM_G; j++) {
			for (k = 0; k < IO_TP_NUM; k++) {
				offset += scnprintf(
					buf + offset, buf_len - offset,
					"%llu%s",
					iom_blk->iogroup_latency_stat[i][j][k],
					k + 1 == IO_TP_NUM ?
						(j == TIME_OVERSIZE_G ? "" :
									"\t") :
						"/");
...
static int io_measure_blk_sched_group_proc_open(struct inode *inode,
						struct file *file)
{
	return single_open(file, io_measure_blk_sched_group_proc_show,
			   pde_data(inode));
}
...
void io_measure_blk_sched_log(void)
{
	...
	for (i = 0; i < TIME_DIV_NUM - 1; i++) {
		if (i != (TIME_DIV_NUM - 2))
			...
			offset += scnprintf(buf + offset, buf_len - offset,
					    "GE%u:[",
					    IOM_TIME_IDX_TO_MS(i - 1));

		for (j = 0; j < IO_TP_NUM; j++)
			offset += scnprintf(
				buf + offset, buf_len - offset,
				(j + 1 == IO_TP_NUM) ? "%llu] " : "%llu/",
				latency_cur[i][j] -
					iom_blk->latency_last[i][j]);
	}
...
static int io_measure_blk_sched_proc_show(struct seq_file *m, void *v)
{
	...
	for (j = 0; j < IO_TP_NUM; j++)
		offset += scnprintf(
			buf + offset, buf_len - offset,
			(j + 1 == IO_TP_NUM) ? "%llu" : "%llu/",
			IOM_NS_TO_US((io_cnt[j] == 0) ?
					     0 :
					     iom_blk->latency_total_time[j] /
						     io_cnt[j]));
...
static int io_measure_blk_sched_proc_open(struct inode *inode,
					  struct file *file)
{
	return single_open(file, io_measure_blk_sched_proc_show,
			   pde_data(inode));
}
...
#else
static void io_measure_blk_rq_insert(void *ignore, struct request_queue *q,
				     struct request *rq)
#endif
{
	if (!rq) {
		iom_pr_info("blk_rq_insert request null error.\n");
		return;
	}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
	pid_t rq_pid = current->pid;
	uint64_t rq_time_us = ktime_get_ns() / 100000;

	rq->android_oem_data1 = ((uint64_t)rq_pid << 48) |
				(rq_time_us & 0x0000FFFFFFFFFFFF);
#else
...
#else
static void io_measure_blk_rq_issue(void *ignore, struct request_queue *q,
				    struct request *rq)
#endif
{
	struct io_measure_blk *iom_blk = NULL;
	...
	if (!rq) {
		iom_pr_info("blk_rq_issue:request null error.\n");
		return;
	}
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
	if ((rq->android_oem_data1 & 0x0000FFFFFFFFFFFF) == 0)
#else
	...
	now_ns = ktime_get_ns();
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
	u64 now_us = ktime_get_ns() / 100000;

	if (now_us <= (rq->android_oem_data1 & 0x0000FFFFFFFFFFFF))
		return;
	delta = (now_us - (rq->android_oem_data1 & 0x0000FFFFFFFFFFFF)) *
		100000;
#else
	...
	if (iolatency_usertype == IOM_USER_NOT_COMMERCIAL) {
		pid_t cur_pid = rq->android_oem_data1 >> 48;

		if (delta >= BLOCK_LATENCY_THRESHOLD &&
		    (rq->cmd_flags & REQ_HW_VIP) &&
		    (cur_pid > 0 && cur_pid <= PID_MAX_LIMIT)) {
			struct task_struct *task = NULL;

			task = find_task_by_vpid_safely(cur_pid);
			if (task) {
				get_task_struct(task);
				block_io_num++;
				if ((block_io_num % LONG_IO_INTERVAL) == 1) {
					LOG_JANK_D(
						JLID_LONG_IO_LATENCY,
						"#ARG1:<%s>#ARG2:<%llu>#TY:<%s>#NU:<%lu>",
						task->comm, IOM_NS_TO_MS(delta),
						BLOCK_LATENCY, block_io_num);
				}
				iom_pr_info(
					"block appeared long IO, delta = %llums, comm = %s, num = %lu\n",
					IOM_NS_TO_MS(delta), task->comm,
					block_io_num);

				put_task_struct(task);
			} else {
				iom_pr_info(
					"Failed to find task with PID: %u\n",
					cur_pid);
			}
		}
	}
	...
	for_each_bio(biop) {
		...
		if (biop && biop->bi_css) {
			...
			if (blkcg != &blkcg_root)
#else
		if (biop && biop->bi_blkg && biop->bi_blkg->blkcg &&
		    biop->bi_blkg->blkcg != &blkcg_root) {
			blkcg = biop->bi_blkg->blkcg;
#endif
			{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
				p_oem_data = (struct blkcg_oem_data *)&blkcg
						     ->android_oem_data1;
				type = p_oem_data->type;
#else
				type = blkcg->type;
#endif
				grp_idx = type < grp_idx ? type : grp_idx;
			}
		}
		rcu_read_unlock();
	}
	...
	if (unlikely(delta > IOM_BLK_LTCY_THRESHOLD)) {
		pr_info("[io_measure]rq_insert_to_issue:rq_flag:0x%x,latency:%lluns.\n",
			rq->cmd_flags, delta);
	}
}
...
#endif
{
	...
	if (!rq) {
		iom_pr_info("blk_rq_complete:request null error.\n");
		return;
	}
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
	if ((rq->android_oem_data1 & 0x0000FFFFFFFFFFFF) == 0)
		return;
	rq_start_ns = (rq->android_oem_data1 & 0x0000FFFFFFFFFFFF) * 100000;
	rq->android_oem_data1 = 0;
#else
	if (rq->rq_start_time_ns == 0)
		return;
...
static void io_measure_blk_register_tracepoints(void)
{
	...
	ret |= register_trace_block_rq_complete(io_measure_blk_rq_complete,
						NULL);
...
static int io_measure_scsi_sched_proc_open(struct inode *inode,
					   struct file *file)
{
	return single_open(file, io_measure_scsi_sched_proc_show,
			   pde_data(inode));
}
...
void io_measure_scsi_dispatch_cmd_start_lifetime(void *ignore,
						 struct scsi_cmnd *cmd,
						 unsigned long long secd,
...
#endif
{
	...
	if (!cmd) {
		iom_pr_info("scsi cmd_start:scsi_cmnd null error.\n");
		return;
	}

	cmd_tag = scsi_cmd_to_rq(cmd)->tag;
	if ((cmd_tag < 0) || (cmd_tag > IOMT_CMD_TAG_STORE_LEN) ||
	    (!iom_scsi_time)) {
		iom_pr_info("scsi cmd_start:invalid value, tag=%d.\n", cmd_tag);
		return;
	}

	iom_scsi_time->iom_start_time[cmd_tag].ticks = jiffies;
	iom_scsi_time->iom_start_time[cmd_tag].ktime = ktime_get_ns();
#else
	if (!cmd) {
		iom_pr_info("scsi cmd_start:scsi_cmnd null error.\n");
		return;
					      ...
					      struct scsi_cmnd *cmd)
{
	...
	if (!cmd || !rq) {
		iom_pr_info("scsi cmd_done:scsi_cmnd/request null error.\n");
		return;
	}
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
	cmd_tag = rq->tag;
	if ((cmd_tag < 0) || (cmd_tag > IOMT_CMD_TAG_STORE_LEN) ||
	    (!iom_scsi_time)) {
		iom_pr_info("scsi cmd_done:invalid value, tag=%d.\n", cmd_tag);
		return;
	}

	if (iom_scsi_time->iom_start_time[cmd_tag].ktime == 0)
#else
	...
	now_ns = ktime_get_ns();
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
	if (now_ns <= iom_scsi_time->iom_start_time[cmd_tag].ktime)
		return;
	delta = now_ns - iom_scsi_time->iom_start_time[cmd_tag].ktime;
#else
	...
	if (unlikely(delta > IOM_SCSI_LTCY_THRESHOLD)) {
		pr_info("[io_measure]scsi_scsi_cmd_done:io_type:%d,latency:%lluns.\n",
			io_type, delta);
	}
	iom_ltcy->iom_latency_add(iom_ltcy, chunksz_idx,
				  (u32)IOM_NS_TO_US(delta), io_type);
...
#ifdef CONFIG_HONOR_RWLOG
	unregister_trace_scsi_dispatch_cmd_start_lifetime(
		io_measure_scsi_dispatch_cmd_start_lifetime, NULL);
#else
	unregister_trace_scsi_dispatch_cmd_start(
		io_measure_scsi_dispatch_cmd_start, NULL);
#endif
	unregister_trace_scsi_dispatch_cmd_done(
		io_measure_scsi_dispatch_cmd_done, NULL);
	tracepoint_synchronize_unregister();
#endif
}
...
#ifdef CONFIG_HONOR_RWLOG
	ret |= register_trace_scsi_dispatch_cmd_start_lifetime(
		io_measure_scsi_dispatch_cmd_start_lifetime, NULL);
#else
	ret |= register_trace_scsi_dispatch_cmd_start(
		io_measure_scsi_dispatch_cmd_start, NULL);
#endif
	ret |= register_trace_scsi_dispatch_cmd_done(
		io_measure_scsi_dispatch_cmd_done, NULL);
					  ...
					  u32 rw_flag)
{
	...
	trace_android_rvh_ogki_check_task_tags(current, &current_vip);
	if (current_vip && blk_is_topio())
		vip_flag = 1;
	else
		vip_flag = 0;
	...
	strncpy(iom_item->curr, current->comm, MAX_COMM_LEN - 1);
	iom_item->vip_flag = vip_flag;
}

static inline struct io_measure_fs_item *
io_measure_fs_item_get(struct io_measure_fs *iom_fs, struct inode *inode,
...
static void io_measure_fs_dataread_start(void *ignore, struct inode *inode,
					 loff_t offset, int bytes, pid_t pid,
...
static void io_measure_fs_dataread_end(void *ignore, struct inode *inode,
				       loff_t offset, int bytes)
{
	struct io_measure_fs *iom_fs = NULL;
	struct io_measure_latency *iom_ltcy = NULL;
	struct io_measure_fs_item *iom_item = NULL;
	...
	if ((now_us <= iom_item->start_time_us) ||
	    (iom_item->start_time_us == 0))
		return;
	delta = now_us - iom_item->start_time_us;

	if (iolatency_usertype == IOM_USER_NOT_COMMERCIAL) {
		if (delta >= FS_DATAREAD_LATENCY_THRESHOLD &&
		    iom_item->vip_flag == 1 && strcmp(iom_item->curr, "appthread-read") != 0) {
			fsread_io_num++;
			if ((fsread_io_num % LONG_IO_INTERVAL) == 1) {
				LOG_JANK_D(
					JLID_LONG_IO_LATENCY,
					"#ARG1:<%s>#ARG2:<%u>#TY:<%s>#NU:<%lu>",
					iom_item->curr, IOM_US_TO_MS(delta),
					FS_DATAREAD_LATENCY, fsread_io_num);
			}
			iom_pr_info(
				"fs_dataread appeared long IO, delta = %ums, comm = %s, num = %lu\n",
				IOM_US_TO_MS(delta), iom_item->curr,
				fsread_io_num);
		}
	}
...
static void io_measure_fs_datawrite_start(void *ignore, struct inode *inode,
					  loff_t offset, int bytes, pid_t pid,
					  char *pathname, char *command)
{
	struct io_measure_fs *iom_fs = NULL;
...
static void io_measure_fs_datawrite_end(void *ignore, struct inode *inode,
					loff_t offset, int bytes)
{
	struct io_measure_fs *iom_fs = NULL;
	struct io_measure_latency *iom_ltcy = NULL;
	...
	if (iolatency_usertype == IOM_USER_NOT_COMMERCIAL) {
		if (delta >= FS_DATAWRITE_LATENCY_THRESHOLD &&
		    iom_item->vip_flag == 1 && strcmp(iom_item->curr, "appthread-read") != 0) {
			fswrite_io_num++;
			if ((fswrite_io_num % LONG_IO_INTERVAL) == 1) {
				LOG_JANK_D(
					JLID_LONG_IO_LATENCY,
					"#ARG1:<%s>#ARG2:<%u>#TY:<%s>#NU:<%lu>",
					iom_item->curr, IOM_US_TO_MS(delta),
					FS_DATAWRITE_LATENCY, fswrite_io_num);
			}
			iom_pr_info(
				"fs_datawrite appeared long IO, delta = %ums, comm = %s, num = %lu\n",
				IOM_US_TO_MS(delta), iom_item->curr,
				fswrite_io_num);
		}
	}
...
{
	unregister_trace_f2fs_dataread_start(io_measure_fs_dataread_start,
					     NULL);
	unregister_trace_f2fs_dataread_end(io_measure_fs_dataread_end, NULL);
	unregister_trace_f2fs_datawrite_start(io_measure_fs_datawrite_start,
					      NULL);
	unregister_trace_f2fs_datawrite_end(io_measure_fs_datawrite_end, NULL);
	tracepoint_synchronize_unregister();
}
...
static void io_measure_fs_register_tracepoints(void)
{
	...
	ret |= register_trace_f2fs_dataread_start(io_measure_fs_dataread_start,
						  NULL);
	ret |= register_trace_f2fs_dataread_end(io_measure_fs_dataread_end,
						NULL);
	ret |= register_trace_f2fs_datawrite_start(
		io_measure_fs_datawrite_start, NULL);
	ret |= register_trace_f2fs_datawrite_end(io_measure_fs_datawrite_end,
						 NULL);
...
static int io_measure_pgcache_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(
		...
		"fs_dataread_ionum:        %lu\n"
		"fs_datawrite_ionum:       %lu\n"
		"block_ionum:              %lu\n",
		...
		IOM_PAGE_TO_KB(iom_pgcache->cache_size /
			       (u64)iom_pgcache->cache_count),
		fsread_io_num, fswrite_io_num, block_io_num);
	return 0;
}
...
static int io_measure_pagecache_init(void)
{
	...
	memset(&(iom_ctrl->iom_pgcache), 0, sizeof(struct io_measure_pgcache));
	ret = io_measure_pgcache_proc_creat(&(iom_ctrl->iom_pgcache));
...
static void io_measure_reset(void)
{
	...
	if ((iom_ctrl == NULL) || (iom_ctrl->iom_blk == NULL) ||
	    (iom_ctrl->iom_fs == NULL) || (iom_ctrl->iom_scsi == NULL))
		return;
	...
	/* fs measure reset */
	iom_fs = iom_ctrl->iom_fs;
	memset(iom_fs->iom_latency.latency_avg, 0,
	       sizeof(iom_fs->iom_latency.latency_avg));
	memset(iom_fs->iom_latency.latency_stat, 0,
	       sizeof(iom_fs->iom_latency.latency_stat));

	/* block measure reset */
	iom_blk = iom_ctrl->iom_blk;
	memset(iom_blk->latency_total_time, 0,
	       sizeof(iom_blk->latency_total_time));
	memset(iom_blk->latency_all, 0, sizeof(iom_blk->latency_all));
	memset(iom_blk->latency_last, 0, sizeof(iom_blk->latency_last));
	memset(iom_blk->iom_latency.latency_avg, 0,
	       sizeof(iom_blk->iom_latency.latency_avg));
	memset(iom_blk->iom_latency.latency_stat, 0,
	       sizeof(iom_blk->iom_latency.latency_stat));
#ifdef CONFIG_HONOR_BLK_CGROUP_IOSMART
	memset(iom_blk->iogroup_latency_stat, 0,
	       sizeof(iom_blk->iogroup_latency_stat));
#endif

	/* scsi measure reset */
	iom_scsi = iom_ctrl->iom_scsi;
	memset(iom_scsi->latency_total_time, 0,
	       sizeof(iom_scsi->latency_total_time));
	memset(iom_scsi->latency_all, 0, sizeof(iom_scsi->latency_all));
	memset(iom_scsi->latency_last, 0, sizeof(iom_scsi->latency_last));
	memset(iom_scsi->iom_latency.latency_avg, 0,
	       sizeof(iom_scsi->iom_latency.latency_avg));
	memset(iom_scsi->iom_latency.latency_stat, 0,
	       sizeof(iom_scsi->iom_latency.latency_stat));
...
static void io_measure_work_run(void)
{
	...
	iom_ctrl->iom_pgcache.cache_size += cached;
	iom_ctrl->iom_pgcache.cache_count++;
}
...
static void io_measure_delay_work(struct work_struct *work)
{
	...
	case IOM_INIT_STAGE1:
		...
		iolatency_usertype = iom_usertype;
		if (iom_usertype == IOM_USER_COMMERCIAL) {
			proc_entry = proc_create(USER_IOM_ENABLE, 0644,
						 iom_ctrl->iom_proc_root,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
	iom_scsi_time = kmalloc(sizeof(struct iom_scsi_time_store), GFP_KERNEL);
	if (iom_scsi_time == NULL) {
		iom_pr_info("io_measure init kmalloc iom_scsi_time failed.\n");
		return -ENOMEM;
	}

	if (memset(iom_scsi_time, 0, sizeof(struct iom_scsi_time_store)) < 0) {
		iom_pr_info("io_measure init memset iom_scsi_time failed.\n");
		return -ENOMEM;
	}
#endif
	iom_ctrl->iom_enable = false;
