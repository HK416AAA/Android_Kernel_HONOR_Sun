		...
		const struct usb_ctrlrequest *ctrl, unsigned number)
{
	...
	for (tmp = 0; tmp < MAX_CONFIG_INTERFACES; tmp++) {
		...
		if (result < 0) {
			INFO(cdev, "interface %d (%s/%p) alt 0 --> %d\n",
					tmp, f->name, f, result);
	...
	INFO(cdev, "set config power %d Attributes 0x%X result 0x%X\n",
				power, c ? c->bmAttributes : 0, result);
	return result;
}
...
static void composite_setup_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status || req->actual != req->length)
		INFO((struct usb_composite_dev *) ep->driver_data,
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_SET_CONFIGURATION:
		...
		INFO(cdev, "USB_REQ_SET_CONFIGURATION: value=0x%X\n", value);
		break;
	...
	case USB_REQ_SET_INTERFACE:
		...
		INFO(cdev, "USB_REQ_SET_INTERFACE: value=0x%X\n", value);
		spin_unlock(&cdev->lock);
		break;
...
unknown:
		...
		if (cdev->use_os_string && cdev->os_desc_config &&
			...
			INFO(cdev, "%s OS descriptors handling\n", __func__);
