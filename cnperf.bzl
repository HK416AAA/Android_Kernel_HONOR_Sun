...
rule_base = "kernel_aarch64_cnperf"
...
def define_cnperf():
    kernel_build_config(
        ...
        srcs = [
            ...
            "build.config.gki_cnperf.aarch64",
            _gen_config_without_source_lines("build.config.gki.aarch64", rule_base),
        ],
    )

    native.filegroup(
        name = rule_base + "_all_kmi_symbol_lists",
        srcs = native.glob(
            ["android/abi_gki_aarch64*"],
            exclude = ["**/*.xml", "**/*.stg", "**/*.allowed_breaks"],
        ),
    )
    ...
    kernel_build(
        ...
        additional_kmi_symbol_lists = [":{}_all_kmi_symbol_lists".format(rule_base)],
    )
