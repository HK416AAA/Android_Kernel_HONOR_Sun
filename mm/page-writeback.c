...
static unsigned long get_dirty_shortlife_pages_sum(struct bdi_writeback *wb)
{
	...
	static unsigned long bdi_recently_sleep_count = 0;
	struct backing_dev_info *bdi = wb->bdi;

	/*
	 * If a task was rate throttled in balance_dirty_pages() within the last
	 * three seconds or so, use that to indicate a higher cleaning rate.
	 */
	if (time_before(jiffies, bdi->last_bdp_sleep + 3 * HZ)) {
		++bdi_recently_sleep_count;
		pr_info("bdi task recent wait, last bdi sleep:%lu, count:%lu\n",
			bdi->last_bdp_sleep, bdi_recently_sleep_count);
		return 0;
	}

	if (!list_empty((struct list_head *)wb->android_oem_data1)) {
		spin_lock(&wb->list_lock);
	...
	/* Not ignore shorlife dirty while shortlife dirty pages beyond 160M */
