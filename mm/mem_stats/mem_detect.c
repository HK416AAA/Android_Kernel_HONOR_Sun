...
void isolated_detect(int file, unsigned long inactive, unsigned long isolated)
{
	...
	if (isolated > inactive) {
		nr_anomal_isolate++;
		mem_anomal_report(file > 0 ? "file_isolated_anomal" :
					     "anon_isolated_anomal",
...
{
	unsigned long active_anon, inactive_anon, active_file, inactive_file,
		free;
	...
	if (free < free_page_min) {
		nr_anomal_free++;
		mem_anomal_report("free_anomal", free, free_page_min,
				  &nr_anomal_free);
	} else if ((active_anon + inactive_anon) > used_page_max) {
		nr_anomal_anon++;
		mem_anomal_report("anon_anomal", (active_anon + inactive_anon),
				  used_page_max, &nr_anomal_anon);
	} else if ((active_file + inactive_file) > used_page_max) {
		nr_anomal_file++;
		mem_anomal_report("file_anomal", (active_file + inactive_file),
				  used_page_max, &nr_anomal_file);
	}
}
