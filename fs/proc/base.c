					...
					size_t count, loff_t *ppos)
{
	...
	if (lock_task_sighand(task, &flags)) {
		die_catch_flags = task->signal->android_oem_data1;
		unlock_task_sighand(task, &flags);
	}
					...
					size_t count, loff_t *ppos)
{
	...
	task->signal->android_oem_data1 = (unsigned short)die_catch_flags;
...
#endif
#ifdef CONFIG_HONOR_DIE_CATCH
	REG("unexpected_die_catch", 0646, proc_unexpected_die_catch_operations),
#endif
...
#endif
#ifdef CONFIG_HONOR_THREAD_STACK_STAT
	REG("thread_stat", 0200, proc_thread_stat_operations),
#endif
};
