...
static void clean_proc_thread_stack(struct proc_thread_stack *proc_stack_entry)
{
	...
	memset(proc_stack_entry, 0, sizeof(*proc_stack_entry));
}
			...
			struct proc_thread_stack *proc_stack_entry)
{
	...
	strscpy(proc_stack_entry->proc_name, task->comm,
		min(sizeof(task->comm), (size_t)(TASK_COMM_LEN)));
...
static int store_stack_cache_from_buf(struct file *file, const char *buf_pos, size_t kstack_size)
{
	...
	memcpy(thread_stack->stack_info, buf_pos, kstack_size);
...
static unsigned long get_number_from_buf(char **buf, bool update_pos)
{
	...
	number_buf = *buf;
	if (number_buf == NULL) {
		pr_err("%s: number_buf null\n", __func__);
		return num_ret;
	}

	next_line = strnchr(number_buf, MAGIC_NUMBER_STR_LEN + 1, '\n');
	if (next_line) {
		*next_line = '\0';
		if (!is_valid_number_len(strlen(number_buf))) {
			pr_err("%s: number len invalid\n", __func__);
			return num_ret;
		}
		...
		size_t count, loff_t *ppos)
{
	...
	if ((count > STACK_LOG_MAX_SIZE) || (count < MAGIC_NUMBER_STR_LEN + 1)) {
		pr_err("%s: count = %zu, too large or too small\n", __func__, count);
		return err;
	}
	...
	if (copy_from_user(kbuf, buf, count)) {
		pr_err("%s: copy to kbuf error, count = %zu\n", __func__, count);
		err = -EFAULT;
		goto out;
	}
	pr_debug("%s: kbuf: %s, ppos:%lld\n", __func__, kbuf, *ppos);
	...
	pr_info("%s: count = %zu, ppos:%lld, stack_size:%zu, err:%d\n", __func__,
				...
				size_t count, loff_t *ppos)
{
	...
	if (!is_valid_index(cache_index)) {
		pr_err("%s: invalid cache index: %d\n", __func__, cache_index);
		ret = -EINVAL;
		goto out;
	}
	...
	pr_info("%s: count=%zu, stacklen=%zu, ppos=%lld, ret(size) = %d\n", __func__, count,
