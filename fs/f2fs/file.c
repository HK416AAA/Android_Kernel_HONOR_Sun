...
{
	trace_android_vh_ogki_f2fs_dsm(name, len);
}

void f2fs_dsm_get(unsigned long code, char *err_msg)
{
	trace_android_vh_ogki_f2fs_dsm_get(code, err_msg);
}
#endif
...
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "F2FS_GOING_DOWN_NEED_FSCK need fsck");
#endif
...
out:
	...
	if (ret >= 0) {
		...
		set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "%s: partial blocks were released i_ino=%lx "
		...
		unsigned int *reserved_blocks)
{
	...
	while (count) {
		...
		f2fs_i_compr_blocks_update(dn->inode, compr_blocks + reserved, true);
