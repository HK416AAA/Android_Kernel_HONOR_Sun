...
void f2fs_decompress_cluster(struct decompress_io_ctx *dic, bool in_task)
{
	...
	if (!ret && (fi->i_compress_flag & BIT(COMPRESS_CHKSUM))) {
		...
		if (provided != calculated) {
			...
			f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "provided != calculated need fsck");
#endif
		}
	}
...
out:
	...
	set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
	f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "access invalid cluster, ino:%lu, nid:%u, ofs_in_node:%u, reason:%s",
		dn->inode->i_ino, dn->nid, dn->ofs_in_node, reason);
#endif
	return true;
#else
