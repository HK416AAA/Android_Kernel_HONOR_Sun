...
#ifdef CONFIG_HONOR_F2FS_SMART_DISCARD
/*
...
#ifdef CONFIG_HONOR_F2FS_BLKFRAG
unsigned int f2fs_get_seg_fragscore(struct f2fs_sb_info *sbi, unsigned int segno)
{
	...
	prev = bitmap[SIT_VBLOCK_MAP_SIZE - 1] >> 1
		| (bitmap[SIT_VBLOCK_MAP_SIZE - 2] << (BITS_PER_BYTE - 1));
	next = bitmap[SIT_VBLOCK_MAP_SIZE - 1] << 1 | (bitmap[SIT_VBLOCK_MAP_SIZE - 1] & 1);
	score += hweight8(bitmap[SIT_VBLOCK_MAP_SIZE - 1] ^ prev)
		+ hweight8(bitmap[SIT_VBLOCK_MAP_SIZE - 1] ^ next);
...
void f2fs_print_valid_map(struct f2fs_sb_info *sbi, unsigned int segno, struct seq_file *seq)
{
	...
	if (seq)
		seq_printf(seq,
		"[SegMap] segno.%u fragmentation record score=%u validmap print......\n",
		segno, se->frag_score);
	else
		pr_err("[SegMap] segno.%u fragmentation record score=%u validmap print......",
		segno, se->frag_score);
	...
	for (row = 0; row < MAP_ROW_LIMIT; ++row) {
		...
		for (col = 0; col < MAP_COL_LIMIT; ++col) {
			sprintf(mapline + MAP_COL_DELIM * col, " "BYTE_TO_BINARY_PATTERN,
				BYTE_TO_BINARY(writable[MAP_COL_WIDTH * row + col],
				readable[MAP_COL_WIDTH * row + col]));
		}
		if (seq)
	...
	if (seq) {
		seq_printf(seq,
		"[SegMap] segno.%u fragmentation recalc score=%u validmap finish......\n",
		segno, f2fs_get_seg_fragscore(sbi, segno));
		seq_printf(seq, "[SegMap]\n");
	} else {
		pr_err("[SegMap] segno.%u fragmentation recalc score=%u validmapfinish......",
		segno, f2fs_get_seg_fragscore(sbi, segno));
		pr_err("[SegMap]");
	}
}
#endif /* CONFIG_HONOR_F2FS_BLKFRAG */
...
static void update_sit_entry(struct f2fs_sb_info *sbi, block_t blkaddr, int del)
{
	...
	if (del > 0) {
		...
		mir_exist = f2fs_test_and_clear_bit(offset,
						se->cur_valid_map_mir);
		if (unlikely(exist != mir_exist)) {
			f2fs_err(sbi, "Inconsistent error when clearing bitmap, blk:%u, old
				bit:%d", blkaddr, exist);
			f2fs_bug_on(sbi, 1);
		}
#endif
...
int f2fs_trim_fs(struct f2fs_sb_info *sbi, struct fstrim_range *range)
{
	 ...
	 */
#ifndef CONFIG_HONOR_F2FS_SMART_DISCARD
	if (f2fs_realtime_discard_enable(sbi))
		goto out;
#endif
...
static void do_write_page(struct f2fs_summary *sum, struct f2fs_io_info *fio)
{
	...
	/* Recalculate hotness type */
	if (fio->type == DATA && (!page_private_gcing(fio->page)) && type <= CURSEG_COLD_DATA) {
		if (__is_valid_data_blkaddr(fio->old_blkaddr))
			type_old = get_seg_entry(fio->sbi,
			GET_SEGNO(fio->sbi, fio->old_blkaddr))->type;
		type = hotness_decide(fio, type_old, type, &value);
	}
#endif
...
int f2fs_inplace_write_data(struct f2fs_io_info *fio)
{
	...
	if (!IS_DATASEG(get_seg_entry(sbi, segno)->type)) {
		...
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "%s: incorrect segment(%u) type, run fsck to fix.",
			  __func__, segno);
#endif
		...
		for (; start < end && start < MAIN_SEGS(sbi); start++) {
			...
			if (is_set_ckpt_flags(sbi, CP_TRIMMED_FLAG)) {
				...
				if (se->valid_blocks == sbi->blocks_per_seg) {
					se->frag_score = BLKFRAG_UNWRITABLE_WEIGHT
