...
static int do_read_inode(struct inode *inode)
{
	...
	if (!sanity_check_inode(inode, node_page)) {
		...
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "!sanity_check_inode need fsck");
#endif
...
struct inode *f2fs_iget(struct super_block *sb, unsigned long ino)
{
	...
	if (!(inode->i_state & I_NEW)) {
		if (is_meta_ino(sbi, ino)) {
			...
			set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
			f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "inaccessible inode: %lu, run fsck to repair", ino);
#endif
			ret = -EFSCORRUPTED;
	...
	if (!err) {
		...
		if (err == -ENOENT) {
			...
			if (is_inode_flag_set(inode, FI_DIRTY_INODE)) {
				...
				set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
				f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "f2fs_evict_inode: inconsistent node id, ino:%lu",
					inode->i_ino);
#endif
			}
		}
