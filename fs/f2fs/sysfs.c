...
GC_THREAD_RW_ATTR(gc_preference, gc_preference);
#endif
...
/*
 * blk_frag_weight is the calculation weight of fragmentation rate, which is an experience
 * value. A total of ten digits are reserved for the fragmentation rate, with 7 digits after
 * the decimal point. The maximum fragmentation rate is 1.
...
#ifdef CONFIG_HONOR_F2FS_BLKFRAG
	proc_create_single_data("fragmentation_info", 0444, sbi->s_proc,
				fragmentation_info_seq_show, sb);
	proc_create_single_data("fragmentation_rate", 0444, sbi->s_proc,
				fragmentation_rate_show, sb);
	proc_create_single_data("fragmentation_segmap", 0444, sbi->s_proc,
				fragmentation_segmap_seq_show, sb);
	proc_create_single_data("fragmentation_hist", 0444, sbi->s_proc,
