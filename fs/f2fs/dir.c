	...
	while (bit_pos < d->max) {
		...
		if (de->name_len == 0) {
			if (found_valid_dirent || !bit_pos) {
				...
				f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "F2FS-fs (%s): invalid namelen(0), ino:%u, run fsck to fix.",
					sbi->sb->s_id, le32_to_cpu(de->ino));
#endif
			}
			bit_pos++;
		...
		if (unlikely(bit_pos > d->max ||
			...
			f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK,
				"corrupted namelen=%d, run fsck to fix.", le16_to_cpu(de->name_len));
#endif
			err = -EFSCORRUPTED;
