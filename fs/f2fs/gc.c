		...
		struct node_info *dni, block_t blkaddr, unsigned int *nofs)
{
	...
	if (sum->version != dni->version) {
		...
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "%s: valid data with mismatched node version.",
			  __func__);
#endif
	}
	...
	if (source_blkaddr != blkaddr) {
		...
		if (unlikely(check_valid_map(sbi, segno, offset))) {
			if (!test_and_set_bit(segno, SIT_I(sbi)->invalid_segmap)) {
				...
				set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
				f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "mismatched blkaddr %u (source_blkaddr %u) in seg %u",
					 blkaddr, source_blkaddr, segno);
#endif
			}
		}
...
recover_out:
	...
	if (err) {
		set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "resize_fs failed, should run fsck to repair!");
#endif
