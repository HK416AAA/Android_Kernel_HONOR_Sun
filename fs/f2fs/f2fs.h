...
#define TEMP_TYPE_NUM NR_CURSEG_DATA_TYPE
#define N_CLUSTERS NR_CURSEG_DATA_TYPE			/* this num can change to other */

/* hotness count info */
struct hotness_info {
	struct radix_tree_root hotness_rt_array[TEMP_TYPE_NUM];
	struct mutex hotness_tree_lock[TEMP_TYPE_NUM];

	atomic_t count; // number of hotness entry
	unsigned int new_blk_cnt;
	unsigned int upd_blk_cnt;
	unsigned int rmv_blk_cnt;
	unsigned int ipu_blk_cnt;
	unsigned int opu_blk_cnt;
	atomic_t blk_cnt_temp[TEMP_TYPE_NUM];
...
struct f2fs_sb_info {
	...
	atomic_t total_writed_block_count; // warm data block write count
						...
						block_t count)
{
	...
	if (unlikely(inode->i_blocks < sectors)) {
		...
		set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "Inconsistent i_blocks, ino:%lu, iblocks:%llu, sectors:%llu",
			  inode->i_ino,
			  (unsigned long long)inode->i_blocks,
			  (unsigned long long)sectors);
#endif
					...
					struct inode *inode, bool is_inode)
{
	...
	if (unlikely(!sbi->total_valid_block_count ||
		...
		set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "dec_valid_node_count: inconsistent block counts, total_valid_block:%u, total_valid_node:%u",
			  sbi->total_valid_block_count,
			  sbi->total_valid_node_count);
#endif
	} else {
	...
	if (is_inode) {
		...
		if (unlikely(inode->i_blocks == 0)) {
			...
			set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
			f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "dec_valid_node_count: inconsistent i_blocks, ino:%lu, iblocks:%llu",
				  inode->i_ino,
				  (unsigned long long)inode->i_blocks);
#endif
			return;
