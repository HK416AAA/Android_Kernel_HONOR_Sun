...
int f2fs_check_nid_range(struct f2fs_sb_info *sbi, nid_t nid)
{
	if (unlikely(nid < F2FS_ROOT_INO(sbi) || nid >= NM_I(sbi)->max_nid)) {
		...
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "%s: out-of-range nid=%x, run fsck to fix.",
			  __func__, nid);
#endif
...
static int truncate_dnode(struct dnode_of_data *dn)
{
	...
	if (IS_INODE(page) || ino_of_node(page) != dn->inode->i_ino) {
		...
		set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "incorrect node reference, ino: %lu, nid: %u, ino_of_node: %u",
				dn->inode->i_ino, dn->nid, ino_of_node(page));
#endif
...
struct page *f2fs_new_node_page(struct dnode_of_data *dn, unsigned int ofs)
{
	...
	if (unlikely(new_ni.blk_addr != NULL_ADDR)) {
		...
		set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "new_ni.blk_addr != NULL_ADDR need fsck");
#endif
		f2fs_handle_error(sbi, ERROR_INVALID_BLKADDR);
		goto fail;
	}
...
page_hit:
	...
	set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
	f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "inconsistent node block, nid:%lu, node_footer[nid:%u,ino:%u,ofs:%u,cpver:%llu,blkaddr:%u]",
			  nid, nid_of_node(page), ino_of_node(page),
			  ofs_of_node(page), cpver_of_node(page),
			  next_blkaddr_of_node(page));
#endif
		...
		if (!test_bit_le(NAT_BLOCK_OFFSET(nid),
			...
			if (ret) {
				...
				if (ret == -EFSCORRUPTED) {
					...
					set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
					f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "NAT is corrupt, run fsck to fix it");
#endif
