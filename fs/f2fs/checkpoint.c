...
out_handle:
	...
	f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "Inconsistent error blkaddr:%u, sit bitmap:%d",
		blkaddr, exist);
#endif
	return exist;
}
					...
					block_t blkaddr, int type)
{
	...
	case DATA_GENERIC_ENHANCE_UPDATE:
		if (unlikely(blkaddr >= MAX_BLKADDR(sbi) ||
			...
			set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
			f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "access invalid blkaddr:%u", blkaddr);
#endif
			dump_stack();
			goto err;
		} else {
...
err_out:
	set_sbi_flag(sbi, SBI_NEED_FSCK);
#ifdef CONFIG_HONOR_F2FS_DSM
	f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "orphan failed (ino=%x), run fsck to fix", ino);
#endif
