...
static int f2fs_remount(struct super_block *sb, int *flags, char *data)
{
...
#ifdef CONFIG_HONOR_F2FS_HOTNESS_CLUSTERING
	bool need_restart_hc = false, need_stop_hc = false;
#endif
	...
	if ((*flags & SB_RDONLY)) {
		...
		need_restart_hc = true;
	} else if (!sbi->hc_thread) {
...
restore_hc:
	if (need_restart_hc) {
		if (f2fs_start_hc_thread(sbi))
			f2fs_warn(sbi, "hc thread has stopped");
	} else if (need_stop_hc) {
		f2fs_stop_hc_thread(sbi);
	}
#endif
restore_gc:
...
try_onemore:
	...
	if (__is_set_ckpt_flags(F2FS_CKPT(sbi), CP_FSCK_FLAG)) {
		...
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "__is_set_ckpt_flags need fsck");
	}
#else
	if (__is_set_ckpt_flags(F2FS_CKPT(sbi), CP_FSCK_FLAG))
		set_sbi_flag(sbi, SBI_NEED_FSCK);
	...
	if (!test_opt(sbi, DISABLE_ROLL_FORWARD) &&
		...
		if (need_fsck) {
			set_sbi_flag(sbi, SBI_NEED_FSCK);
			f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "need_fsck set need fsck");
		}
#else
		if (need_fsck)
			set_sbi_flag(sbi, SBI_NEED_FSCK);
...
free_nm:
#ifdef CONFIG_HONOR_F2FS_HOTNESS_CLUSTERING
	/*hi init fail will not affect fill super,but filee super error should free hi */
	f2fs_destory_hc_manager(sbi);
#endif
