		...
		if (si->sbi->hi) {
			...
			if (si->sbi->centers_valid) {
				for (j = 0; j < si->sbi->n_clusters; ++j)
					seq_printf(s, "centers[%u] = %u, ", j,
					si->sbi->centers[j]);
				seq_printf(s, "\n");
			}
			seq_printf(s, "hot : count = %u, IRR_min = %u, irr_max = %u\n",
				atomic_read(&si->sbi->hi->blk_cnt_temp[HOT]),
				si->sbi->hi->irr_min[HOT],
				si->sbi->hi->irr_max[HOT]);
			seq_printf(s, "warm: count = %u, IRR_min = %u, irr_max = %u\n",
				atomic_read(&si->sbi->hi->blk_cnt_temp[WARM]),
				si->sbi->hi->irr_min[WARM],
				si->sbi->hi->irr_max[WARM]);
			seq_printf(s, "cold: count = %u, IRR_min = %u, irr_max = %u\n",
				atomic_read(&si->sbi->hi->blk_cnt_temp[COLD]),
				si->sbi->hi->irr_min[COLD],
				si->sbi->hi->irr_max[COLD]);
		}
#endif
	}
...
static int dsm_f2fs_show(struct seq_file *s, void *v)
{
	...
	f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "f2fs set need fsck and report dsm log");
	return 0;
}
