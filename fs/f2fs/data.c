...
#ifdef CONFIG_HONOR_BLK_CGROUP_IOSMART
struct oem_data_share {
	unsigned int	bios;
	unsigned int	binder_flag;
};
#endif

#ifdef CONFIG_WB_PIN_FILE
...
static void f2fs_trace_read_submit_io(struct bio *bio)
{
	...
	if (bio && (bio->bi_io_vec != NULL)) {
		...
		if (first_page != NULL && first_page->mapping != NULL) {
			...
			path = f2fs_trace_get_file_name(pathbuf,
							MAX_TRACE_PATHBUF_LEN,
							first_page->mapping->host);
			trace_f2fs_dataread_start(
				first_page->mapping->host,
...
static void f2fs_trace_read_endio(struct bio *bio)
{
	...
	if (bio->bi_io_vec != NULL) {
		first_page = bio->bi_io_vec[0].bv_page;
		if ((first_page != NULL) && (first_page->mapping != NULL))
			trace_f2fs_dataread_end(first_page->mapping->host,
						  page_offset(first_page),
						  bio->bi_iter.bi_size);
	}
}
...
static void f2fs_submit_write_bio(struct f2fs_sb_info *sbi, struct bio *bio,
				  enum page_type type)
{
#if defined(CONFIG_HONOR_BLK_CGROUP_IOSMART)
	struct oem_data_share *p_oem_data;
	WARN_ON_ONCE(is_read_io(bio_op(bio)));
	if (!(current->flags & PF_MUTEX_GC)) {
		p_oem_data = (struct oem_data_share *)
					&current->android_oem_data1[5];
		p_oem_data->bios++;
	}
#else
	WARN_ON_ONCE(is_read_io(bio_op(bio)));
...
#ifdef CONFIG_HONOR_IO_MEASURE
	if (trace_f2fs_datawrite_start_enabled() && !IS_NOQUOTA(inode)) {
		char *path, pathbuf[MAX_TRACE_PATHBUF_LEN];

		path = f2fs_trace_get_file_name(pathbuf,
						    MAX_TRACE_PATHBUF_LEN,
						    inode);
		trace_f2fs_datawrite_start(inode, pos, len,
						 current->pid, path,
						 current->comm);
	}
#endif
			...
			struct page *page, void *fsdata)
{
	...
	if (trace_f2fs_datawrite_end_enabled())
		trace_f2fs_datawrite_end(inode, pos, len);
#endif
